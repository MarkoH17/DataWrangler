...
namespace DataWrangler.Forms
{
    public partial class EditRecordType : MetroForm
    {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (_removedAttrs.Count > 0) needsDbAction = true;
        ...
        public void LoadHistory()
        {
            ...
            {
                NotificationHelper.ShowNotification(this, NotificationHelper.NotificationType.Error,
                    "Failed to load history for this record type. Please try again.");
            }
        }
        ...
        private void RemoveRow(object sender)
        {
            ...
            for (var i = rowNum + 1; i < tableLayoutPanel1.RowCount; i++)
            for (var j = 0; j < tableLayoutPanel1.ColumnCount; j++)
            {
                var control = tableLayoutPanel1.GetControlFromPosition(j, i);
                if (control != null) tableLayoutPanel1.SetRow(control, i - 1);
            }


            var attributeLabels = tableLayoutPanel1.Controls.OfType<MetroLabel>().Where(x => x.Name.Contains("Row"))
                .OrderBy(x => x.Text);
            var newLabelCtr = 1;
